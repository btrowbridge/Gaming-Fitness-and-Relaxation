...
public class JointOrientation : MonoBehaviour
{
	...
	public bool countingCurl = false;
	public bool countingPunch = false;
	public float w;
	public float x;
	public float y;
	public float z;
	...
	void Start()
	{
		curlTot = PlayerPrefs.GetInt("curlTot");
    ...
    void Update ()
    {
		workDur += Time.deltaTime;

        // Access the ThalmicMyo component attached to the Myo object.
        ThalmicMyo thalmicMyo = myo.GetComponent<ThalmicMyo> ();

        // Update references when the pose becomes double tap or the r key is pressed.
        ...
        if (updateReference && (-5 <= roll) && (roll <= 5)) {
        ...
        roll = rollFromZero (zeroRoll, myo.transform.forward, myo.transform.up);
	...
	void CountCurl()
	{
		...
		if (!countingCurl && (x<=-0.2) && (y<=-0.2)) //condition that triggers the start of the counting the start of counting
		{
			countingCurl = true; //begin counting
		}
		
		//While counting
		else if (countingCurl && (x>=0.0) && (y >= 0.0))//triggers that end the counting period
		{
			curls++;    //adds to number of repititions
			curlText.text = curls.ToString();
	...
	{
		//while we are not counting
		if (!countingPunch && (z >= 0.4) && (roll >= 30)) //condition that triggers the start of the counting the start of counting
		{
			countingPunch = true; //begin counting
		}
		
		//While counting
		else if (countingPunch && (z <= 0.1) && (roll<=5))//triggers that end the counting period
		{
			punches++;    //adds to number of repititions
			punchText.text = punches.ToString(); 
	...
	void OnDisable()
	{
		//calculate results to game stats
		int results = punches + curls;
		GameMaster.instance.AddToAmmo (results);

		//set new playerprefs
		PlayerPrefs.SetInt ("curlTot",curlTot + curls);
		PlayerPrefs.SetInt ("punchTot", punchTot + punches);
		PlayerPrefs.SetFloat ("workDur",workDur);
